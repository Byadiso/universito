...
function createPostHtml(postData, largeFont = false) {
  ...
  var likeButtonActiveClass = postData.likes.includes(userLoggedIn._id)
    ? "active"
    : "";
  var retweeetButtonActiveClass = postData.retweetUsers.includes(
    userLoggedIn._id
  )
    ? "active"
    : "";
...
function outputChatsList(chatList, container) {
  ...
  if (chatList.length == 0) {
    container.append(
      "<span class='noResults'>Nothing to show Go and start a conversation with your universito friends</span>"
    );
  }
}
